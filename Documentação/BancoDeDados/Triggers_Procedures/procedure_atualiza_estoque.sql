...
BEGIN
    ...
    SELECT SUM( quantidade ) INTO total_entradas FROM lanchonete_t92.entradas_estoque 
		WHERE produtos_id_produto = produto;
        
	# Pegar a qtdade de produtos na tabela de saídas e somá-las
    SELECT SUM( quantidade ) INTO total_saidas FROM lanchonete_t92.saidas_estoque 
		WHERE produtos_id_produto = produto;
    ...
    SELECT COUNT(*) INTO existe_registro FROM lanchonete_t92.estoque 
		WHERE produtos_id_produto = produto;
...
IF ( existe_registro = 0 )  THEN
     INSERT INTO lanchonete_t92.estoque (
		produtos_id_produto,
        quantidade,
        id_criador
     ) VALUES (
...
ELSE
	UPDATE lanchonete_t92.estoque SET quantidade = total_entradas - total_saidas
		WHERE produtos_id_produto = produto;
END IF ;
